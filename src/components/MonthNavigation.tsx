      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div className="flex items-center justify-between gap-4">
          ...
          <button
            ...
            className={`shrink-0 w-10 h-10 rounded-sm flex items-center justify-center transition-all duration-200 ${
              activeMonthIndex === 0
          ...
          <div className="flex-1 relative overflow-hidden">
            <div className="flex items-center justify-start gap-2 overflow-x-auto scrollbar-hide py-2 px-2 sm:px-4">
              {months.map((month, index) => (
              <button
                type="button"
                key={`${month.monthName}-${month.year}`}
                onClick={() => onMonthChange(index)}
                className={`relative shrink-0 px-4 py-2.5 rounded-sm text-sm font-medium transition-all duration-300 ease-out ${
